...
arruma_dados_rigeo <- function(folhas = "inputs/campo/folhas.shp"){
  ...
  dir <- "inputs/projetos/"
  arquivos_rigeo <- list.files(dir, pattern="\\.zip$", full.names=TRUE)
  ...
  for(i in 1:length(arquivos_rigeo)) {
    ...
    filtered_df_bateia <- dplyr::filter(x, grepl("Mineralometria Concentrado de Bateia",
                                                 Name, ignore.case = TRUE),fixed=TRUE)[1,1]
    filtered_df_bateia2 <- dplyr::filter(x, grepl("Mineralometria_Concentrado_de_Bateia",
                                                  Name, ignore.case = TRUE))[1,1]
    filtered_df_bateia3 <- dplyr::filter(x, grepl("Mineralometria de Concentrado de Bateia",
                                                  Name, ignore.case = TRUE))[1,1]
    filtered_df_bateia4 <- dplyr::filter(x, grepl("Mineralometria_de_Concentrado_de_Bateia",
                                                  Name, ignore.case = TRUE))[1,1]
    filtered_df_bateia <- c(filtered_df_bateia, filtered_df_bateia2,
                            filtered_df_bateia3, filtered_df_bateia4)
    filtered_df_bateia <- filtered_df_bateia[!is.na(filtered_df_bateia)]

    if(length(filtered_df_bateia) != 0){
      concentrado_filtro <- unzip(arquivos_rigeo[i], filtered_df_bateia)
      if( !is.na(filtered_df_bateia)){
        data_cb <- readxl::read_excel(concentrado_filtro, col_types = "text")
        ...
        data_cb_join <- left_join( data_cb, campo, by= c("num_lab" = "Num_Lab"))
        names(data_cb_join) <- toupper(names(data_cb_join))
        names(data_cb_join) <- stringr::str_replace(names(data_cb_join), "_PCT", "")

        data_cb_join <- as.data.frame(data_cb_join)
        colnames(data_cb_join) <- gsub("PIR..OXIDADA", "PIR.OXIDADA", colnames(data_cb_join))

        res_cb[[i]] <- data_cb_join
      }
    }

    filtered_df_sedimento <- filter(x, grepl("Geoquimica Sedimento de Corrente",
                                             Name, ignore.case = TRUE))[1,1]
    filtered_df_sedimento2 <- filter(x, grepl("Geoquimica de Sedimento de Corrente",
                                             Name, ignore.case = TRUE))[1,1]
    filtered_df_sedimento3 <- filter(x, grepl("Geoquimica_Sedimento_de_Corrente",
                                              Name, ignore.case = TRUE))[1,1]
    filtered_df_sedimento4 <- filter(x, grepl("Analise_Qu¡mica_Sedimento_de_Corrente",
                                              Name, ignore.case = TRUE))[1,1]

    filtered_df_sedimento <- c(filtered_df_sedimento,filtered_df_sedimento2,
                               filtered_df_sedimento3, filtered_df_sedimento4)
    filtered_df_sedimento <- filtered_df_sedimento[!is.na(filtered_df_sedimento)]
    ...
    filtered_campo7 <- filter(x, grepl(".shx",
                                       Name, ignore.case = TRUE))[1,1]
    filtered_campo <- c(filtered_campo,filtered_campo2,
                        filtered_campo3,filtered_campo4,
                        filtered_campo5,filtered_campo6, filtered_campo7)
    ...
    if(length(filtered_df_sedimento) != 0){
      ...
      if(!is.na(filtered_df_sedimento)){
        ...
        unzip(zipfile = arquivos_rigeo[i], filtered_campo, exdir = temp2 )
        ...
        data_sc_join <- dplyr::inner_join(data_sc, campo, by= c( "num_lab" = "Num_Lab"))
        names(data_sc_join) <- toupper(names(data_sc_join))
        data_sc_join <- as.data.frame(data_sc_join)
        colnames(data_sc_join) <- gsub("SC._PPM", "SC_PPM", colnames(data_sc_join))
        res_sc[[i]] <-  data_sc_join
      }
    }

    filtered_df_bateia_gq <- filter(x, grepl("Geoquimica Concentrado de Bateia",
                                             Name, ignore.case = TRUE))[1,1]
    filtered_df_bateia_gq2 <- filter(x, grepl("Geoquimica_Concentrado_de_Bateia",
                                              Name, ignore.case = TRUE))[1,1]
    filtered_df_bateia_gq3 <- filter(x, grepl("Analise_Qu¡mica_Concentrado_de_Bateia",
                                              Name, ignore.case = TRUE))[1,1]
    filtered_df_bateia_gq <- c(filtered_df_bateia_gq, filtered_df_bateia_gq2,
                               filtered_df_bateia_gq3)
    filtered_df_bateia_gq <- filtered_df_bateia_gq[!is.na(filtered_df_bateia_gq)]

    if(length(filtered_df_bateia_gq) != 0) {
      concentrado_filtro_gq <- unzip(arquivos_rigeo[i],filtered_df_bateia_gq)
      if( !is.na(filtered_df_bateia_gq)){
        data_cb_gq <- readxl::read_excel(concentrado_filtro_gq, col_types = "text")
        unzip(zipfile = arquivos_rigeo[i], filtered_campo, exdir = temp3)
    ...
    filtered_df_solo <- filter(x, grepl("Geoquimica Solo",
                                        Name, ignore.case = TRUE))[1,1]
    filtered_df_solo2 <- filter(x, grepl("Geoquimica de Solo",
                                        Name, ignore.case = TRUE))[1,1]
    ...
    filtered_df_solo <- c(filtered_df_solo,filtered_df_solo2,
                          filtered_df_solo3, filtered_df_solo4)
    ...
    if(length(filtered_df_solo) != 0){
      ...
      if(!is.na(filtered_df_solo)){
        ...
        unzip(zipfile = arquivos_rigeo[i], filtered_campo, exdir = temp4 )
        ...
        data_l_join <- dplyr::inner_join(data_l, campo, by= c( "num_lab" = "Num_Lab"))
    ...
    filtered_df_rocha <- filter(x, grepl("Geoquimica Rocha",
                                         Name, ignore.case = TRUE))[1,1]
    filtered_df_rocha2 <- filter(x, grepl("Geoquimica de Rocha",
                                         Name, ignore.case = TRUE))[1,1]
    ...
    filtered_df_rocha <- c(filtered_df_rocha,filtered_df_rocha2,
                           filtered_df_rocha3, filtered_df_rocha4)
    filtered_df_rocha <- filtered_df_rocha[!is.na(filtered_df_rocha)]
    ...
    if(length(filtered_df_rocha) != 0){
      ...
      if(!is.na(filtered_df_rocha)){
        ...
        unzip(zipfile = arquivos_rigeo[i], filtered_campo, exdir = temp5 )
        ...
        data_r_join <- dplyr::inner_join(data_r, campo, by= c( "num_lab" = "Num_Lab"))
    ...
    filtered_df_minerio <- filter(x, grepl("Geoquimica_Mineral_Minerio",
                                           Name, ignore.case = TRUE))[1,1]
    filtered_df_minerio2 <- filter(x, grepl("Geoquimica de Mineral Minerio",
                                            Name, ignore.case = TRUE))[1,1]
    filtered_df_minerio3 <- filter(x, grepl("Analise_Qu¡mica_Mineral_Minerio",
                                            Name, ignore.case = TRUE))[1,1]

    filtered_df_minerio <- c(filtered_df_minerio,filtered_df_minerio2,
                             filtered_df_minerio3)
    filtered_df_minerio <- filtered_df_minerio[!is.na(filtered_df_minerio)]
    ...
    if(length(filtered_df_minerio) != 0){
      ...
      if(!is.na(filtered_df_minerio)){
        ...
        unzip(zipfile = arquivos_rigeo[i], filtered_campo, exdir = temp6 )
        ...
        data_m_join <- dplyr::inner_join(data_m, campo, by= c( "num_lab" = "Num_Lab"))
...
if(res_sc > 0){
  ...
  tables_sc <- data.frame(lapply(tables_sc, function(x) gsub(".", ",", x,
                                                             fixed = TRUE)))
...
#Organizar nomes dos campos para Sedimento de corrente
  ...
  ## Prepara coordenadas
  tables_sc$LONGITUDE <- as.numeric(gsub(",", ".", tables_sc$LONGITUDE,
                                         fixed = TRUE))
  tables_sc$LATITUDE <- as.numeric(gsub(",", ".", tables_sc$LATITUDE,
                                        fixed = TRUE))
  tables_sc[is.na(tables_sc$LATITUDE), "LATITUDE"] <- tables_sc[is.na(tables_sc$LATITUDE), "LAT"]
  tables_sc[is.na(tables_sc$LONGITUDE), "LONGITUDE"] <- tables_sc[is.na(tables_sc$LONGITUDE), "LONG"]
  ...
  spdf <- sf::st_as_sf(x = tables_sc,
                   coords = c("LONGITUDE", "LATITUDE"),
                   crs = "+proj=longlat +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +no_defs", remove = FALSE)
  ...
  selecionadas <- c("ID", "NUM_LAB", "layer", "LONGITUDE", "LATITUDE",
                    "NUM_CAMPO", "PROJETO", "CLASSE", "Lab")
  ...
  write.table(estacoes_folhas_sc , "outputs/sc_tidy.csv",
              row.names = FALSE,na = "", sep = ";", dec = ".", quote = TRUE)
  #Pivoteia e une as bases
  lista_pivo[[1]] <- tidyr::pivot_longer(estacoes_folhas_sc, cols=11:ncol(estacoes_folhas_sc),
                              names_to = "Analito", values_to = "Valor")
}
  ## Arrumar dados de Concentrado de Bateia - Mineralometria--------------------
  if(res_cb > 0){  tables_cb <- do.call(plyr::rbind.fill, res_cb)
  tables_cb <- data.frame(lapply(tables_cb, function(x) gsub(".", ",", x,
                                                             fixed = TRUE)))
  colnames(tables_cb)
  write.csv2(tables_cb, "outputs/integrada_rigeo_cb.csv",
   ...
   tables_cb <- tables_cb %>%
    tidyr::unite("Lab",   LEITURA, ABERTURA, sep = " - ", na.rm = TRUE)
  dfp <- tidyr::pivot_longer(data = tables_cb,
  ...
  dfp$value <- stringr::str_replace(dfp$value, "75-100", "75 - 100 %")
  ...
  ## Prepara coordenadas
  df_cb_tidy$LONGITUDE <- as.numeric(gsub(",", ".", df_cb_tidy$LONGITUDE,
                                          fixed = TRUE))
  df_cb_tidy$LATITUDE <- as.numeric(gsub(",", ".", df_cb_tidy$LATITUDE,
                                         fixed = TRUE))
  ...
  estacoes_folhas_cb <- data.frame(estacoes_folhas_cb[, c(1:9)] ,BASE, estacoes_folhas_cb[, 10:ncol(estacoes_folhas_cb)])

  colnames(estacoes_folhas_cb)[3] <- "AREA"
  colnames(estacoes_folhas_cb)[2] <- "N_LAB"
  colnames(estacoes_folhas_cb )[4:5] <- c("LONG", "LAT")

  write.table(estacoes_folhas_cb , "outputs/cb_tidy.csv",
              row.names = FALSE, na = "", dec = ",", sep = ";")
  #Pivoteia e une as base
  lista_pivo[[2]] <- tidyr::pivot_longer(estacoes_folhas_cb, cols=11:ncol(estacoes_folhas_cb),
                              names_to = "Analito", values_to = "Valor")
}
  ## Arrumar dados de Concentrado de Bateia - Análises químicas-----------------
  if(res_cb_gq > 0){tables_cb_gq <- do.call(plyr::rbind.fill, res_cb_gq)
  tables_cb_gq <- data.frame(lapply(tables_cb_gq, function(x) gsub(".", ",", x,
                                                                   fixed = TRUE)))
  write.csv2(tables_cb_gq, paste0("outputs/",
                                  "integrada_rigeo_cb_gq.csv"), row.names = FALSE)
  ...
  ## Prepara coordenadas
  tables_cb_gq$LONGITUDE <- as.numeric(gsub(",", ".", tables_cb_gq$LONGITUDE,
                                            fixed = TRUE))
  tables_cb_gq$LATITUDE <- as.numeric(gsub(",", ".", tables_cb_gq$LATITUDE,
                                           fixed = TRUE))
  tables_cb_gq[is.na(tables_cb_gq$LATITUDE), "LATITUDE"] <- tables_cb_gq[is.na(tables_cb_gq$LATITUDE), "LAT"]
  tables_cb_gq[is.na(tables_cb_gq$LONGITUDE), "LONGITUDE"] <- tables_cb_gq[is.na(tables_cb_gq$LONGITUDE), "LONG"]
  ## Extrai códigos das folhas das estações (wgs84)
  crs_SIRGAS2000 <- "+proj=longlat +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +no_defs"
  xy <- tables_cb_gq[,c("LONGITUDE", "LATITUDE")]
  spdf <- sf::st_as_sf(x = tables_cb_gq,
                   coords = c("LONGITUDE", "LATITUDE"),
                   crs = "+proj=longlat +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +no_defs", remove = FALSE)
  spdf <- sf::st_transform(spdf, crs = crs_SIRGAS2000)
  folhas_po <- sf::st_transform(folhas_po, crs = crs_SIRGAS2000)
  ...
  ## Salva arquivo final do Rigeo
  write.table(estacoes_folhas_cb_gq , "outputs/cb_gq_tidy.csv",
              row.names = FALSE,na = "", sep = ";", dec = ".", quote = TRUE)
  #Pivoteia e une as bases
  lista_pivo[[3]] <- tidyr::pivot_longer(estacoes_folhas_cb_gq, cols=11:ncol(estacoes_folhas_cb_gq),
                                 names_to = "Analito", values_to = "Valor")
}
  ## Arrumar dados de Rocha - Análises químicas---------------------------------
  if(res_r > 0){tables_r <- do.call(plyr::rbind.fill, res_r)
  tables_r <- data.frame(lapply(tables_r, function(x) gsub(".", ",", x,
                                                           fixed = TRUE)))
  write.csv2(tables_r, paste0("outputs/",
                              "integrada_rigeo_r.csv"), row.names = FALSE)
  ...
  ## Prepara coordenadas
  tables_r$LONGITUDE <- as.numeric(gsub(",", ".", tables_r$LONGITUDE,
                                        fixed = TRUE))
  tables_r$LATITUDE <- as.numeric(gsub(",", ".", tables_r$LATITUDE,
                                       fixed = TRUE))
  tables_r[is.na(tables_r$LATITUDE), "LATITUDE"] <- tables_r[is.na(tables_r$LATITUDE), "LAT"]
  tables_r[is.na(tables_r$LONGITUDE), "LONGITUDE"] <- tables_r[is.na(tables_r$LONGITUDE), "LONG"]
  ## Extrai códigos das folhas das estações (wgs84)
  crs_SIRGAS2000 <- "+proj=longlat +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +no_defs"
  xy <- tables_r[,c("LONGITUDE", "LATITUDE")]
  spdf <- sf::st_as_sf(x = tables_r,
                   coords = c("LONGITUDE", "LATITUDE"),
                   crs = "+proj=longlat +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +no_defs", remove = FALSE)
  spdf <- sf::st_transform(spdf, crs = crs_SIRGAS2000)
  folhas_po <- sf::st_transform(folhas_po, crs = crs_SIRGAS2000)
  ...
  ## Salva arquivo final do Rigeo
  write.table(estacoes_folhas_r , "outputs/r_tidy.csv",
              row.names = FALSE,na = "", sep = ";", dec = ".", quote = TRUE)
  #Pivoteia e une as base
  rlista_pivo[[4]] <- tidyr::pivot_longer(estacoes_folhas_r, cols=11:ncol(estacoes_folhas_r),
                             names_to = "Analito", values_to = "Valor")
}
  ## Arrumar dados de Solo - Análises químicas----------------------------------
  if(res_l > 0){tables_l <- do.call(plyr::rbind.fill, res_l)
  tables_l <- data.frame(lapply(tables_l, function(x) gsub(".", ",", x,
                                                           fixed = TRUE)))
  write.csv2(tables_l, paste0("outputs/",
                              "integrada_rigeo_l.csv"), row.names = FALSE)
  ...
  ## Prepara coordenadas
  tables_l$LONGITUDE <- as.numeric(gsub(",", ".", tables_l$LONGITUDE,
                                        fixed = TRUE))
  tables_l$LATITUDE <- as.numeric(gsub(",", ".", tables_l$LATITUDE,
                                       fixed = TRUE))
  tables_l[is.na(tables_l$LATITUDE), "LATITUDE"] <- tables_l[is.na(tables_l$LATITUDE), "LAT"]
  tables_l[is.na(tables_l$LONGITUDE), "LONGITUDE"] <- tables_l[is.na(tables_l$LONGITUDE), "LONG"]
  ## Extrai códigos das folhas das estações (wgs84)
  crs_SIRGAS2000 <- "+proj=longlat +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +no_defs"
  xy <- tables_l[,c("LONGITUDE", "LATITUDE")]
  spdf <- sf::st_as_sf(x = tables_l,
                   coords = c("LONGITUDE", "LATITUDE"),
                   crs = "+proj=longlat +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +no_defs", remove = FALSE)
  spdf <- sf::st_transform(spdf, crs = crs_SIRGAS2000)
  folhas_po <- sf::st_transform(folhas_po, crs = crs_SIRGAS2000)
  ...
  ## Salva arquivo final do Rigeo
  write.table(estacoes_folhas_l , "outputs/l_tidy.csv",
              row.names = FALSE,na = "", sep = ";", dec = ".", quote = TRUE)
  #Pivoteia e une as bases
  lista_pivo[4] <- tidyr::pivot_longer(estacoes_folhas_l, cols=11:ncol(estacoes_folhas_l),
                             names_to = "Analito", values_to = "Valor")
}
  ## Arrumar dados de  Minerio - Análises químicas------------------------------
  if(res_m > 0){tables_m <- do.call(plyr::rbind.fill, res_m)
  tables_m <- data.frame(lapply(tables_m, function(x) gsub(".", ",", x,
                                                           fixed = TRUE)))
  write.csv2(tables_m, paste0("outputs/",
                              "integrada_rigeo_m.csv"), row.names = FALSE)
  ...
  ## Prepara coordenadas
  tables_m$LONGITUDE <- as.numeric(gsub(",", ".", tables_m$LONGITUDE,
                                        fixed = TRUE))
  tables_m$LATITUDE <- as.numeric(gsub(",", ".", tables_m$LATITUDE,
                                       fixed = TRUE))
  tables_m[is.na(tables_m$LATITUDE), "LATITUDE"] <- tables_m[is.na(tables_m$LATITUDE), "LAT"]
  tables_m[is.na(tables_m$LONGITUDE), "LONGITUDE"] <- tables_m[is.na(tables_m$LONGITUDE), "LONG"]
  ## Extrai códigos das folhas das estações (wgs84)
  crs_SIRGAS2000 <- "+proj=longlat +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +no_defs"
  xy <- tables_m[,c("LONGITUDE", "LATITUDE")]
  spdf <- sf::st_as_sf(x = tables_m,
                   coords = c("LONGITUDE", "LATITUDE"),
                   crs = "+proj=longlat +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +no_defs", remove = FALSE)
  colnames(spdf)
  spdf <- sf::st_transform(spdf, crs = crs_SIRGAS2000)
  ...
  ## Salva arquivo final do Rigeo
  write.table(estacoes_folhas_m , "outputs/m_tidy.csv",
              row.names = FALSE,na = "", sep = ";", dec = ".", quote = TRUE)
  #Pivoteia e une as base

  lista_pivo[5] <- tidyr::pivot_longer(estacoes_folhas_m, cols=11:ncol(estacoes_folhas_m),
                             names_to = "Analito", values_to = "Valor")
}
  ## Une as bases --------------------------------------------------------------
