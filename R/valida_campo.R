...
#'
#' @examples
valida_campo <- valida_campo <- function(path_ctr, path_base, base, layer_base,
  ...
  cont_am_est <- unida %>% dplyr::group_by(nm_estacao) %>% dplyr::count()
  contagem_amostras <- unida %>% dplyr::group_by(nm_estacao, id_classe) %>% dplyr::count()
  out[[6]] <- cont_am_est[cont_am_est$n == 1,]
  ...
  # Amostras duplicatas sem registro (sedimento de corrente)
  estacoes_sel <- contagem_amostras[contagem_amostras$n>1 &
                                      contagem_amostras$id_classe == "S",
                                    "nm_estacao"]
  ...
  dupli_erro_nome <- unida[!is.na(unida$nm_amostra_extra),
                           "cd_numero_campo"]
